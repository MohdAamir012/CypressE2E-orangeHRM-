...
const userStatus=':nth-child(3) > .oxd-input-group > :nth-child(2) > .oxd-select-wrapper > .oxd-select-text';
const userName='.oxd-form-row >div.oxd-grid-2 :nth-child(4)>div >div>input';
const userPasswd='.user-password-cell > .oxd-input-group > :nth-child(2) > .oxd-input';
const confrimPasswd=':nth-child(2) > .oxd-input-group > :nth-child(2) > .oxd-input';
...
class AdminAddUser{
    addUser(name,passwd,empName,role,status){
      ...
      cy.wait(1000)
      role==='Admin' ?cy.get(userRole).first().type('{downArrow}{enter}'):cy.get(userRole).first().type('{downArrow}{downArrow}{enter}')
  cy.wait(1000)
  ...
  cy.wait(1000)
        ...
        status==='Enabled'?cy.get(userStatus).type('{downArrow}{enter}'):cy.get(userStatus).type('{downArrow}{downArrow}{enter}')
        cy.scrollTo(0)
        cy.wait(1000)
